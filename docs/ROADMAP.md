...
| **1B.5.1** | Authorization model correction: `RoleGrant` carries its own permissions; `grantCarries` reads them; the API-key intersection moves to the key's binding scope. No new endpoints | **none** — `role_permissions` already is the per-grant relation | §6n cases 10–12 and 15 pass; restoring the flattened union fails the suite; all existing tests green and unchanged |
| **1B.5.2** | Target-scope evaluator: `ScopeChainResolver`, `AuthorizationService`, `@RequiresPermission`; `TenancyController` migrated onto it | none | §6n cases 1–9, 13, 14, 25, 30 pass; a chain taken from request input fails the suite |
| **1B.5.3** | Denial auditing: `authorization.denied` written on every refusal, added to `SECURITY_SENSITIVE_AUDIT_ACTIONS` | none — `audit_logs_insert` constrains `acc_app` by tenancy only, not by action vocabulary, so the action is already writable | §6n cases 23–24 pass; removing the write, or recording the attempted scope as the actor's, fails the suite |
...
| **1B.5.7** | Adversarial closure: the remaining §6n matrix, `GET /auth/me/authorization`, mutation table recorded | none | 0 failed, 0 skipped; every mutation in §6n breaks the suite and is restored |
