...
| **1B.5** | Authorization administration: `AuthorizationGuard`, per-grant permission sets, role CRUD, grant/revoke, tenant-role seeding at provisioning | 1B.4 | The full §6b and §6e matrices pass; each service-only escalation guard has a named test |
