...
| `auth` | Request authentication, tenant-context resolution and the authorization decision point: `AuthGuard`, access-token issue/verify, `ScopeResolver`, `X-Acc-Organization` selection, `PermissionEvaluator`, `AuthorizationService` (the composed target-scope check every scoped service calls) and `@RequiresPermission` (the endpoint-level half, necessary but never sufficient — ADR-005 D-1/D-5), auth rate limiting, CSRF. Registered globally so the API denies by default. All of it shipped in Phase 1B.3, including the parts `ROADMAP.md` originally scheduled for 1B.4/1B.5 — the 1B.3 exit criterion is the authenticated chain proven end to end, which is not demonstrable without them (ADR-004 D-1) | `iam`, `audit` |
| `tenancy` | Orgs, resellers, workspaces, teams, RBAC/ABAC. Owns the advisory-identifier cross-check (`AdvisoryTenantGuard`), registered globally so any handler gets it by declaring the identifiers it accepts (`TENANCY.md` §2b, ADR-004 D-3), and `ScopeChainResolver`, which resolves a target's authoritative ancestry from the database for `scopeCovers` (ADR-005 D-5) — a tenancy read, deliberately not placed in `auth`. `PermissionEvaluator` and `AuthorizationService` live in `auth`, alongside the scope resolution they depend on | `iam`, `audit` |
