...
| `auth` | Request authentication, tenant-context resolution and the authorization boundary: `AuthGuard`, access-token issue/verify, `ScopeResolver`, `X-Acc-Organization` selection, `ScopeChainResolver` (a target's authoritative ancestry, read from the database), `PermissionEvaluator` (coherent-grant algebra, and deliberately free of audit and database concerns) and `AuthorizationService` — the single entry point every scoped operation calls, which composes the other two so no handler assembles a target scope and a chain for itself (ADR-005 D-1/D-5). It also **owns denial auditing**: a refusal writes `authorization.denied` through the ordinary `AuditWriter`, in its own transaction, before the refusal is raised (ADR-005 D-6). Neither the evaluator nor the chain resolver knows the audit trail exists. A declarative `@RequiresPermission` is **not** built: target extraction has no established convention yet, and inventing one was rejected in favour of the service (`ROADMAP.md` 1B.5.7). Plus auth rate limiting and CSRF. Registered globally so the API denies by default. All of it shipped in Phase 1B.3, including the parts `ROADMAP.md` originally scheduled for 1B.4/1B.5 — the 1B.3 exit criterion is the authenticated chain proven end to end, which is not demonstrable without them (ADR-004 D-1) | `iam`, `audit` |
| `tenancy` | Orgs, resellers, workspaces, teams, RBAC/ABAC. Owns the advisory-identifier cross-check (`AdvisoryTenantGuard`), registered globally so any handler gets it by declaring the identifiers it accepts (`TENANCY.md` §2b, ADR-004 D-3). The authorization trio — `ScopeChainResolver`, `PermissionEvaluator`, `AuthorizationService` — lives in `auth` beside the scope resolution it depends on; `ScopeResolver` there already reads `organizations`/`workspaces`/`teams` for the same purpose, and placing the chain resolver in `tenancy` would have forced `AuthModule` to import `TenancyModule` and re-ordered global guard registration | `iam`, `audit` |
